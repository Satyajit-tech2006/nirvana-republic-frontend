...
interface SeedInstanceData {
  spoutOffset: THREE.Vector3;
  bowlTarget: THREE.Vector3;
  arcHeight: number;
  pushStrength: number;
  delay: number;
  duration: number;
...
interface SceneHandles {
  ...
  packetPivot: THREE.Group;
  seedMesh: THREE.InstancedMesh;
  seedInstances: SeedInstanceData[];
  contactShadowPacket: THREE.Mesh;
  contactShadowBowl: THREE.Mesh;
  spoutLocalOffset: THREE.Vector3;
  spoutDirLocal: THREE.Vector3;
  dummy: THREE.Object3D;
  tempSpoutWorld: THREE.Vector3;
  tempSpoutDir: THREE.Vector3;
...
const HERO_COPY: HeroCopyContent = {
  ...
  description:
    "Ceremonial seed blends, sun-dried and stone-milled in small batches, because wellness should feel like a ritual, not a routine.",
};
...
function createBowlGeometry(): THREE.LatheGeometry {
  const points: THREE.Vector2[] = [
    ...
    new THREE.Vector2(0.78, 0.07),
    new THREE.Vector2(1.18, 0.27),
    new THREE.Vector2(1.42, 0.5),
    new THREE.Vector2(1.52, 0.62),
    new THREE.Vector2(1.58, 0.65),
    new THREE.Vector2(1.62, 0.7),
    new THREE.Vector2(1.58, 0.75),
    new THREE.Vector2(1.5, 0.73),
    new THREE.Vector2(1.4, 0.69),
...
function generateSeedInstances(
  count: number,
  rng: () => number
): SeedInstanceData[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const bowlTarget = new THREE.Vector3(
      BOWL_CENTER[0] + disk.x,
      BOWL_CENTER[1] + BOWL_BASIN_Y_OFFSET + stackHeight,
      BOWL_CENTER[2] + disk.y
    );
    const spoutOffset = new THREE.Vector3(
      (rng() - 0.5) * 0.12,
      (rng() - 0.5) * 0.05,
      (rng() - 0.5) * 0.12
    );
    ...
    const delay =
      POUR_TILT_START +
      delayBase * (POUR_TILT_HOLD_END - POUR_TILT_START - 0.08);

    instances.push({
      spoutOffset,
      bowlTarget,
      arcHeight: 0.1 + rng() * 0.15,
      pushStrength: 0.8 + rng() * 0.6,
      delay,
      duration: 0.055 + rng() * 0.03,
      scale: 0.75 + rng() * 0.45,
      ...
      rotationSpeed: 2.0 + rng() * 4,
      settleFrequency: 10 + rng() * 5,
      settleDecay: 6 + rng() * 3,
      colorVariant: rng(),
    });
  }
...
function computeTiltAngle(progress: number): number {
  if (progress <= POUR_TILT_START) {
    ...
    return (
      lerp(REST_TILT_RADIANS, REST_TILT_RADIANS + MAX_TILT_ADD_RADIANS, t) +
  ...
  if (progress <= POUR_TILT_HOLD_END) {
    ...
    return (
      REST_TILT_RADIANS +
      MAX_TILT_ADD_RADIANS +
      Math.sin(wobbleT * Math.PI * 8) * 0.01
    );
  }
  const span = 1 - POUR_TILT_HOLD_END;
  const t = easeInOutCubic(
    clamp01(span > 0 ? (progress - POUR_TILT_HOLD_END) / span : 1)
  );
...
function updateSeedInstanceMatrix(
  inst: SeedInstanceData,
  progress: number,
  elapsed: number,
  ...
  const overshoot = clamp01(
    (progress - (inst.delay + inst.duration)) / overshootWindow
  );

  const arriveEase = easeOutCubic(localT);
  const omt = 1 - arriveEase;

  const p0x = spoutWorld.x + inst.spoutOffset.x;
  const p0y = spoutWorld.y + inst.spoutOffset.y;
  const p0z = spoutWorld.z + inst.spoutOffset.z;
  const p1x = p0x + spoutDir.x * inst.pushStrength;
  const p1y = p0y + spoutDir.y * inst.pushStrength;
  const p1z = p0z + spoutDir.z * inst.pushStrength;
  const p3x = inst.bowlTarget.x;
  const p3y = inst.bowlTarget.y;
  const p3z = inst.bowlTarget.z;
  const p2x = p3x;
  const p2y = Math.max(p0y, p3y) + inst.arcHeight;
  const p2z = p3z;

  const posX =
    omt * omt * omt * p0x +
    3 * omt * omt * arriveEase * p1x +
    3 * omt * arriveEase * arriveEase * p2x +
    arriveEase * arriveEase * arriveEase * p3x;
  const posY =
    omt * omt * omt * p0y +
    3 * omt * omt * arriveEase * p1y +
    3 * omt * arriveEase * arriveEase * p2y +
    arriveEase * arriveEase * arriveEase * p3y;
  const posZ =
    omt * omt * omt * p0z +
    3 * omt * omt * arriveEase * p1z +
    3 * omt * arriveEase * arriveEase * p2z +
    arriveEase * arriveEase * arriveEase * p3z;
  ...
  const spinAmount = hasStarted
    ? elapsed * inst.rotationSpeed + overshoot * inst.rotationSpeed * 2
    : 0;
...
function updateScene(
  ...
  handles.tempSpoutWorld
    .copy(handles.spoutLocalOffset)
    .applyMatrix4(handles.packetPivot.matrixWorld);
  handles.tempSpoutDir
    .copy(handles.spoutDirLocal)
    .transformDirection(handles.packetPivot.matrixWorld)
    .normalize();

  for (let i = 0; i < handles.seedInstances.length; i++) {
    const inst = handles.seedInstances[i];
    const matrix = updateSeedInstanceMatrix(
      inst,
      progress,
      elapsed,
      handles.tempSpoutWorld,
    ...
    handles.tempColor
      .copy(handles.colorDark)
      .lerp(handles.colorLight, inst.colorVariant);
    handles.seedMesh.setColorAt(i, handles.tempColor);
  }

  handles.seedMesh.instanceMatrix.needsUpdate = true;
  if (handles.seedMesh.instanceColor)
    handles.seedMesh.instanceColor.needsUpdate = true;

  (handles.contactShadowPacket.material as THREE.MeshBasicMaterial).opacity =
    lerp(0.48, 0.16, clamp01(progress / POUR_TILT_START));
  (handles.contactShadowBowl.material as THREE.MeshBasicMaterial).opacity =
    lerp(0.35, 0.55, progress);
...
async function buildSceneAsync(
  ...
  const bagMeshGroup = await createBagMeshAsync(
    geometries,
    materials,
    textures
  );
  ...
  const spoutLocalOffset = new THREE.Vector3(
    PACKET_WIDTH * 0.12,
    PACKET_HEIGHT * 0.98,
    PACKET_DEPTH * 0.15
  );
  ...
  const packetShadowMaterial = new THREE.MeshBasicMaterial({
    map: shadowTexture,
    transparent: true,
    depthWrite: false,
    opacity: 0.48,
  });
  const bowlShadowMaterial = new THREE.MeshBasicMaterial({
    map: shadowTexture,
    transparent: true,
    depthWrite: false,
    opacity: 0.35,
  ...
  const contactShadowPacket = new THREE.Mesh(
    shadowGeometry,
    packetShadowMaterial
  );
  contactShadowPacket.rotation.x = -Math.PI / 2;
  contactShadowPacket.scale.set(1.6, 1.2, 1);
  contactShadowPacket.position.set(
    PACKET_PIVOT[0],
    PACKET_PIVOT[1] - 0.01,
    PACKET_PIVOT[2]
  );
  ...
  contactShadowBowl.rotation.x = -Math.PI / 2;
  contactShadowBowl.scale.set(2.8, 2.2, 1);
  contactShadowBowl.position.set(
    BOWL_CENTER[0],
    BOWL_CENTER[1] - 0.01,
    BOWL_CENTER[2]
  );
  ...
  const seedMesh = new THREE.InstancedMesh(
    seedGeometry,
    seedMaterial,
    SEED_COUNT
  );
  ...
  return {
    ...
    tempSpoutWorld: new THREE.Vector3(),
    tempSpoutDir: new THREE.Vector3(),
    tempColor: new THREE.Color(),
    colorDark: new THREE.Color(COLORS.seedDark),
    colorLight: new THREE.Color(COLORS.seedLight),
    geometries,
    materials,
    textures,
  };
...
function disposeScene(
  ...
  if (!handles) return;
  if (container.contains(handles.renderer.domElement))
    container.removeChild(handles.renderer.domElement);
  handles.geometries.forEach((g) => g.dispose());
  handles.materials.forEach((m) => m.dispose());
  handles.textures.forEach((t) => t.dispose());
  handles.renderer.dispose();
  handles.renderer.forceContextLoss();
}
...
export function SeedScrollScene() {
      ...
      try {
        ...
        if (isStaticMode) {
          ...
          resizeObserver = new ResizeObserver(() => {
            ...
            if (w === 0 || h === 0) return;
            sceneHandlesRef.current.camera.aspect = w / h;
            sceneHandlesRef.current.camera.updateProjectionMatrix();
            sceneHandlesRef.current.renderer.setSize(w, h);
            updateScene(sceneHandlesRef.current, 1, 0);
          });
        ...
        const handleScroll = () => {
          targetProgress = computeScrollProgress(sectionEl);
        };
        ...
        updateScene(handles, currentProgress, 0);
      } catch (error) {
  ...
  const heroCopy = (
    <div className="relative z-10 max-w-xl">
      <p className="text-xs font-medium uppercase tracking-[0.28em] text-[#8f6a45]">
        {HERO_COPY.eyebrow}
      </p>
      <h1 className="mt-4 font-serif text-4xl leading-tight text-[#2b2620] md:text-5xl">
        {HERO_COPY.title}
      </h1>
      <p className="mt-5 text-base leading-relaxed text-[#4a463d] md:text-lg">
        {HERO_COPY.description}
      </p>

      {/* Conditionally rendered Admin Dashboard CTA */}
      {user?.role === "admin" && (
        <div className="mt-8">
          <Link
            to="/admin"
            className="inline-flex items-center gap-2 rounded-full border border-[#2b2620] bg-[#2b2620] px-7 py-3 text-sm font-medium uppercase tracking-[0.14em] text-[#fffcf7] transition-all duration-300 hover:bg-[#4a463d] hover:border-[#4a463d]"
          >
      ...
      <div
        className={`absolute bottom-[-60px] left-0 flex items-center gap-3 transition-opacity duration-700 ${
          isLoaded ? "opacity-100" : "opacity-0"
        }`}
      >
        <div className="h-8 w-[1px] bg-[#2b2620]/30 animate-pulse"></div>
        <span className="text-[10px] font-medium uppercase tracking-widest text-[#2b2620]/50">
          Scroll to witness the ritual
        </span>
      </div>
    </div>
  );
    ...
    return (
      <section className="relative w-full bg-[#fffcf7]">
        <div className="mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-10 px-6 py-20 md:grid-cols-2 md:py-28">
          ...
          <div
            aria-hidden="true"
            className="relative aspect-[4/3] w-full overflow-hidden rounded-sm bg-[#efebe1] shadow-sm md:aspect-[5/4]"
          >
  ...
  return (
    <section
      ref={sectionRef}
      className="relative h-[220vh] w-full bg-[#fffcf7] md:h-[280vh]"
    >
      <div className="sticky top-0 flex h-screen w-full items-center overflow-hidden">
        <div className="mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-10 px-6 md:grid-cols-2">
          ...
          <div
            ref={containerRef}
            aria-hidden="true"
            className={`relative aspect-[4/3] w-full overflow-hidden rounded-sm bg-[#efebe1]/40 transition-opacity duration-700 md:aspect-[5/4] ${
              isLoaded ? "opacity-100" : "opacity-0"
            }`}
          />
        </div>
